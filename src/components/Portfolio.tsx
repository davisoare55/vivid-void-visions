...
const Portfolio = ({ showFullSite }: PortfolioProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Note: We generally don't remove the Vturb script on unmount because 
      // if the user navigates back, we want it cached/available. 
      // However, if strict cleanup is needed, uncomment the next lines.
          ...
          <div className="relative py-12">
            <div className="overflow-hidden px-4 md:px-16">
              <div ref={carouselRef} className="flex transition-transform duration-500 ease-in-out">
                {projects.map((project, index) => {
                  ...
                  const isVisible = index === currentSlide || index === (currentSlide + 1) % projects.length;
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {projects.map((_, index) => (
                <button
                  key={index}
                  className={`w-3 h-3 rounded-full transition-colors duration-200 ${index === currentSlide ? 'bg-primary' : 'bg-gray-300 hover:bg-primary/50'
                    }`}
                  onClick={() => {
